...
\begin{itemize}
	\item The report and it's source code under /report
...
The source code itself is structured using the follwoing components:
\begin{itemize}
	...
	\item rmq/util.rs: A file for utility functions. I added a min\_of, that is faster at getting the min element from a slice, than the default iterator impl. 
\end{itemize}
...
array of size $O(n^2)$ words. To build the lookup table, the build function iterates the data.
In each iteration a minimum is stored and updated in the nested loop, which iterates from the starting 
index to the end of the dataset. At each sub-iteration either the preceeding minimum is stored, or, if the new 
...
The sparse table implementation stores the minimum for each sliding windows of size $2^l$, 
...
minimum of the two siding windows. 
...
the new table array. The remaining levels are build iterativey. We utilize that in the previous level
...
calculates the level from the difference if the range indicators and can afterwards derive the 
...
the common parent is found. Because the parent is always smaller or equal to the child nodes,
...
to the end of the array. While we did not reach the root node, we take the last level and 
and $k$ preceeding elements, where $k$ is the number of elements in the last created level. 
...
nodes in our tree array. Afterwards we iterate until the left boundary is greater or equal to the right boundary, 
which means we reached the common parent. As described previously at most $\log{n}$ iterations are performed. 
In each iteration we first create a mask that is 0 if the indecies are odd and fully 1 otherwise. Using this mask 
we can update our result value only if the index is odd. To move to the next higher level we divide the indecies by 2.
...
The block based sparse table, works similiar to the standard sparse table, but 
devides the data into blocks of sized size, of which the minima are then stored in a 
sparse table. A reference to the original data allows to answer queries that are not fully covered by the blocks. 
...
The precomputed variant of the block based sparse table works similiar to the previous implementation,
...
Cartesian tree's are used to reduce space usage to linear, while maintaining constant query time.
...
we use a hash map as lookup table for each different tree signature. This avoids redundant computation 
...
the previous sparse table implementation here. The tails and heads in non fully enclosed 
...
The benchmarks were run a MacBook Pro with M1 Pro chip. 
...
has a theoretical query complecity of $O(1)$. The reason for the linear 
...
roughly linearily until it settles at a constant query time, when 
each query has amortized latency of a DRAM access. 

The two largest PrecomputeBlock impementations have their lowest latency at 
around $10^5$ elements. For lower input sizes, the query time is worse than 
other data structures, while for higher input sizes the query time is leading.
\autoref{sec:block_sizes} dicusses possible reasons for this behaviour.

The SegmentTree implementation shows a slight linearily growing slope
...
linearily with $\frac{1}{n}$. The CachedQuery also behaves as expected for 
...
from the expected linear space usage. This might be a result from 
choosing a block size significant different to the theoretical 
...
in exchange has a significant smaller memory footprint.
...
Block sizes linearily shrink the size of the underlying sparse table.
...
For pre-compute block implementations, trades the extra query time of blocks against
additional space for the prefix/suffix arrays. For small N, smaller blocks are better.
For larger N, all implementations have a very similiar query time, making larger blocks
the better choice, as they have smaller space usage. 

For cartesian trees the block size, has to be chosen acording to the expected N.
Larger and smaller block sizes can cause higher memory usage and query time.

\subsection{Absolute Speed Considerations}

The fastest query time for $N=10^7$ is achived by the PrecomputeBlock<512> 
implementation with 13.33ns per query. This value represents roughly 40 cycles
on the ~3GHz cores of the test system and is less than a single memory access 
in DRAM with ~100ns. Because N is larger than the CPU cache could possibly hold,
it is likely that the query time is dominated by parallel memory access throughput.
The used CPU has a OoO pipeline and can hold a couple of fetches in flight, while 
...
could be done by working with smaller indecies rather than the 64 bit values. 
